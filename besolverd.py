...
def main(queryVcfFile,queryBamFile, genomeBuild, nbthreadsStr, fastaGenome, sample,  outputPrefix, mosdepthPath, rtgtoolsPath, bedtoolsPath,dataPath, downloadReference):
    ...
    cmds += [" cat /tmp/results.tmp | cut -f 1 | perl -pe 's/.*minCov([0-9]+)\/$/$1/' > /tmp/left"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-q","--queryVcfFile",help="query vcf file (must be gzipped and indexed)", required=True)
    parser.add_argument("-b","--queryBamFile",help="query bam/cram file (must be indexed)", required=True)
